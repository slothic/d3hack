...
#undef D3HACK_SEASON_EVENT_FIELD
    ...
    struct {
        ...
        int    momentum_autofire_every = 0;    // d3hack-custom: keep Gears of Dreadlands Momentum stacks up while you Strafe (0 = off, 4 is a good value). Strafe's auto-fired primary never grants Momentum, so stacks bleed away doing the one thing the set is for. This blocks the decay and adds a stack every Nth write, at the point the game stores the counter -- so N tunes how fast it climbs, not whether it holds. Capped at the set's own 20.
